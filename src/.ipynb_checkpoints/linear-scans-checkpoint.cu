...
enum class Algorithm
{
    ...
    hubs_ws_opt_block_size, /** A spatio-graph-based approach that builds an index with hubs and lower bounds w/ TensorCore support */
    hubs_ws_shared_memory, /** A spatio-graph-based approach that builds an index with hubs and lower bounds w/ shared memory support */
    hubs_ws_shared_memory_and_BLOCK,
    hubs_ws_gemm, /** A spatio-graph-based approach that builds an index with hubs and lower bounds w/ GEMM support */
...
const char* algorithm_descriptions[static_cast<std::size_t>(Algorithm::Count)] = {
        ...
        "HubsWS_OPT_BLOCK_SIZE -- a spatio-graph index-based approach with Tensor Core optimization",  // add this
        "HubsWS_shared_memory -- a spatio-graph index-based approach with shared memory optimization",  // add this
        "hubs_ws_shared_memory_and_BLOCK ",
        "HubsWS_GEMM -- a spatio-graph index-based approach with GEMM optimization",  // add this
    ...
    -> std::pair< thrust::host_vector< idx_t >, thrust::host_vector< R > >
{
    ...
    switch( alg )
    {
        ...
        case Algorithm::hubs_ws:
            ...
            case Algorithm::hubs_ws_opt_block_size:  // New enum value
#ifdef USE_FAISS
                bitonic_hubs_ws::C_and_Q_Opt_block_size(N, dV, Q, dQ, k,
                                    thrust::raw_pointer_cast(d_knn.data()),
                                    thrust::raw_pointer_cast(d_distances.data()));
#else
            ...
            break;
            // Add new algorithm option:
            case Algorithm::hubs_ws_shared_memory:  // New enum value
#ifdef USE_FAISS
                bitonic_hubs_ws::C_and_Q_shared_memory(N, dV, Q, dQ, k,
                                    thrust::raw_pointer_cast(d_knn.data()),
                                    thrust::raw_pointer_cast(d_distances.data()));
#else
                std::cerr << "Requested algorithm HubsWS_shared_memory but compiled without FAISS support" << std::endl;
                assert( false && "Compiled without faiss support.");
#endif
            break;
            case Algorithm::hubs_ws_shared_memory_and_BLOCK:  // New enum value
#ifdef USE_FAISS
                bitonic_hubs_ws::C_and_Q_shared_memory_Opt_Block(N, dV, Q, dQ, k,
                                    thrust::raw_pointer_cast(d_knn.data()),
                                    thrust::raw_pointer_cast(d_distances.data()));
#else
                std::cerr << "Requested algorithm HubsWS_shared_memory_and_Block but compiled without FAISS support" << std::endl;
                assert( false && "Compiled without faiss support.");
#endif
            break;

            case Algorithm::hubs_ws_gemm:  // New enum value
#ifdef USE_FAISS
                bitonic_hubs_ws::C_and_Q_TensorCore_GEMM(N, dV, Q, dQ, k,
                                    thrust::raw_pointer_cast(d_knn.data()),
                                    thrust::raw_pointer_cast(d_distances.data()));
#else
                std::cerr << "Requested algorithm HubsWS_GEMM but compiled without FAISS support" << std::endl;
                assert( false && "Compiled without faiss support.");
#endif
            break;
