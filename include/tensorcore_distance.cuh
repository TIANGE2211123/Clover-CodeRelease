...
void calculate_distances_batch_tensorcore(
    ...
    // Kernel to gather hub coordinates
    auto gather_hub_coords = [] __device__ (float* temp_hubs, const R* points, 
                                           const idx_t* dH, int hub_idx) {
        if (hub_idx < H) {
...
void smart_distance_calculation(
    ...
    if (!use_tensor_cores) {
        ...
        Calculate_Distances_Original<<<grid_size, block_size>>>(
            batch_id, batch_size, n, dH, distances, points, hub_counts, dH_assignments
        );
        CHECK_ERROR("Calculate_Distances_Original");
    }
}
